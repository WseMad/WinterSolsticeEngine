...
partial class tCprsr
{
	...
	private partial class teLex
	{
		...
		public int cGetRow()
		{
			return e_NewLineCnt + 1;
		}

		public void cRun(tCprsr a_Ownr, string a_Path, bool a_CmbnOnly, ref string a_DftLibDiry, List<string> a_DpdtFlnms,
			ref double a_Len, StringBuilder a_OptBfr, StringBuilder a_RptBfr)
		{
			// 记录实参
			e_Ownr = a_Ownr;
			e_Path = a_Path;
			...
			// 报告缓冲
			e_RptBfr = new StringBuilder();
			
			// 报告
			e_RptBfr.AppendLine("【文件】" + e_Path);

			// 读取源文本，如果为空则立即返回
			e_SrcText = File.ReadAllText(a_Path, Encoding.UTF8);

			if (string.IsNullOrEmpty(e_SrcText))
			{
				return;
			}

			a_Len += (double)e_SrcText.Length;	// 更新长度

			// 如果只合并，追加值输出缓冲
			if (a_CmbnOnly)
			{
				a_OptBfr.Append(e_SrcText);
				a_OptBfr.AppendLine();	// 追加一个换行，以防文件最后一行是行注释，导致下一个文件的首行被注释掉
				return;
			}
			...
			// 如果还需要压缩形参和局部变量名
			if (e_Ownr.e_RunCfg.c_CprsPrmAndLocName)
			{
				// 交由函数压缩器处理
				a_Ownr.e_Fctn.cRun(a_Ownr, e_RptBfr, ref e_TknList);
			}
			...
			if (null != a_DpdtFlnms)
			{
				...
				if (l_AsynIcldIdx < e_TknList.Count)
				{
					...
					if ((null != l_Tkn_LPrth) && (l_Tkn_LPrth.c_Tmnl == tTmnl.i_LPrth) &&
						(null != l_Tkn_LBrkt) && (l_Tkn_LBrkt.c_Tmnl == tTmnl.i_LBrkt))
					{
						// 默认目录
						a_DftLibDiry = e_TknList[l_AsynIcldIdx + 4].c_Attr.ToString();
						a_DftLibDiry = a_DftLibDiry.Substring(1, a_DftLibDiry.Length - 2);
						...
						for (int l_FlnmIdx = l_PairBrkt.Item1 + 1; l_FlnmIdx < l_PairBrkt.Item2; ++l_FlnmIdx)
						{
							...
							{
								// 注意，起始结尾都是引号！
								a_DpdtFlnms.Add(l_Flnm.Substring(1, l_Flnm.Length - 2));
							}
						}
					}
				}
			...
			if (true)
			{
				...
				Console.WriteLine(a_Path + "，共计 = " + (1 + e_NewLineCnt) + " 行");
				...
				for (int i=0; i<e_TknList.Count; ++i)
				{
					...
					if ((tTmnl.i_Smcln == e_TknList[i].c_Tmnl) &&
					...
					else // 如果“关键保留字字面值标识符”后面仍是“关键保留字字面值标识符”，跟上一个空格
				...
				if (l_NewLineTknIdxAry.Count > 0)
				{
					...
					for (int n=0; n<l_NewLineTknIdxAry.Count; ++n)
					{
						...
						var l_NextTkn = (l_Idx < e_TknList.Count) ? e_TknList[l_Idx + 1] : null;
				...
				// 提交输出给压缩器
				a_OptBfr.Append(e_OptStrm.ToString());

				// 提交报告给压缩器
				a_RptBfr.Append(e_RptBfr.ToString());
			}
		}
		...
		private void	eTrimNewLine_Add_ExpOrBlk(List<tTkn> a_Tmp, ref int a_Idx, tTkn a_Tkn_Kwd)
		{
			...
			else // 表达式
			{
				...
				if (teLex.tTmnl.i_Smcln == e_TknList[l_SNC].c_Tmnl)		// 表达式 ;
				{
					// 什么都不做，因为行没有在这里中断
				}
				else
				if (teLex.tTmnl.i_NewLine == e_TknList[l_SNC].c_Tmnl)	// 表达式 换行
				{
					eChgNewLineToSmcln(null, e_TknList, l_SNC);	// 把换行改成分号
				}
				else
				if (teLex.tTmnl.i_RBrc == e_TknList[l_SNC].c_Tmnl)		// 表达式 }
				{
		...
		private void	eScan_StaMchn()
		{
			...
			// 空白
			if (seIsWhiteSpc(e_IptStrm.cGetNextCha()))
			{
				// 跳过
				e_IptStrm.cFch(true);
			}
		...
		private void	eScan_StaMchn_LineCmt()
		{
			...
			else
			{
				// 如何处理注释？
				bool l_Add = false;
				if (e_Ownr.e_RunCfg.c_CprsEnab && ("删除全部" == e_Ownr.e_RunCfg.c_CprsCmtMode))
				{
					//
				}
				else
				if (e_Ownr.e_RunCfg.c_CprsEnab && ("保留全部" == e_Ownr.e_RunCfg.c_CprsCmtMode))
				{
					l_Add = true;
				}
				else
				if (e_Ownr.e_RunCfg.c_CprsEnab && ("保留内联文档" == e_Ownr.e_RunCfg.c_CprsCmtMode))
				{
					l_Add = seIsIlnDoc(l_Line);
				}
		...
		private StringBuilder	e_RptBfr;		// 报告缓冲
		private string			e_SrcText;		// 源文本
